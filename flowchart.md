...
flowchart LR
    ...
    subgraph Scan["📱 Scan"]
        ...
        AutoFill --> ConfirmCheckIn["Confirm Check-In"]
        ConfirmCheckIn -->|"POST /api/checkin"| LogScan["Log QR Scan<br/>+ Create/Update Lot"]
    end
...
```mermaid
flowchart TD
    NewLot["Chemical Lot Created"] --> IsPeroxide{Is peroxide-forming<br/>chemical?}
    ...
    AddToPeroxide --> CalcSchedule["Calculate Test Schedule<br/>(based on config rules)"]
...
```mermaid
erDiagram
    User ||--o{ UserLab : "assigned to"
    User ||--o{ UserLocation : "assigned to"
    User ||--o{ PurchaseRequest : "creates"
    User ||--o{ StockTransaction : "performs"

    Role ||--o{ User : "has"

    Location ||--o{ Lab : "contains"
    Location ||--o{ UserLocation : "assigned"

    Lab ||--o{ UserLab : "assigned"
    Lab ||--o{ InventoryLot : "stored in"
    Lab ||--o{ ItemLabSetting : "configured"

    ItemCategory ||--o{ Item : "categorizes"
    Vendor ||--o{ Item : "supplies"

    Item ||--o{ InventoryLot : "tracked as"
    Item ||--o{ PurchaseRequestItem : "ordered"
    Item ||--o{ ItemLabSetting : "settings"

    InventoryLot ||--o{ StockTransaction : "logged"
    InventoryLot ||--o{ PeroxideTest : "tested"
    InventoryLot ||--o{ ShelfLifeExtension : "extended"
    InventoryLot ||--o{ QrScanLog : "scanned"

    PurchaseRequest ||--o{ PurchaseRequestItem : "contains"
    PurchaseRequest ||--o| PoReference : "assigned"

    PurchaseRequestItem ||--o{ PurchaseRequestItemRevision : "revised"

    PeroxideConfigRule ||--o{ PeroxideTest : "governs"
